...
namespace Cookbook.Client
{
	...
	public class RestClient
	{
		...
		{
			HttpResponseMessage response = await SendAsync(HttpMethod.Get, endpoint, null, customHeaders: customHeaders);
		...
		private async Task<HttpResponseMessage> SendAsync(HttpMethod method, string endpoint, HttpContent? content = null, Dictionary<string, string>? customHeaders = null)
		{
			...
			try
			{
				...
				httpRequest.Headers.Authorization = string.IsNullOrWhiteSpace(JsonWebToken) ? null : new AuthenticationHeaderValue("Bearer", JsonWebToken);
